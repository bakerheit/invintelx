/**
 * The demo dataset: a builder's merchant with four months of trading behind it.
 *
 * This is the fixture only — it builds documents and writes nothing. Two
 * callers use it and they must not drift apart: `pnpm db:seed`, which fills a
 * development database from the command line, and the onboarding endpoint,
 * which is how somebody who has just registered gets a screen with something on
 * it. A second, subtly different catalogue would mean the thing a new user is
 * shown is not the thing anyone develops against.
 *
 * Every document it produces carries `isDemo: true`, which is what makes the
 * whole set removable in one press later.
 */
...
export interface DemoDataset {
  ...
  supplierItems: SupplierItemDoc[];
  movements: MovementDoc[];
}

/**
 * Every document the demo is made of, ready to insert. Pure: it touches no
 * collection and takes its clock and its actor from the caller, so the same
...
export function buildDemoDataset(options: {
  ...
  const { now, actorId, actorName } = options;

  const itemDocs: ItemDoc[] = CATALOG.map((seed, index) => ({
    _id: new ObjectId(),
    sku: seed.sku,
    name: seed.name,
    description: '',
  ...
  // without turning the demo into a warehouse layout exercise.
  ...
  const ledger = itemDocs.flatMap((item) =>
    buildLedger(item, bins, actorId, actorName, now, random),
  );

  return {
    items: itemDocs,
    locations: [site, ...zones, ...bins],
    suppliers: supplierDocs,
    supplierItems: supplyLines,
    movements: ledger,
  };
}
