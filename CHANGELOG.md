...
- The dashboard cannot show incoming deliveries running late. Purchase orders
  now record what is on its way and when it was promised, but the dashboard does
  not read them yet, so the screen still says so where that section would be
  rather than leaving it out and reading as "no delivery is late". The gap is a
  screen reading data that exists, not missing data.
- Purchase orders have no screen. The whole lifecycle — raising a draft,
  sending, receiving a short delivery, cancelling — is API only.
